...
function OauthModel(options = {}) {
  ...
  this.authorizationCodeStore = options.authorizationCodeStore;
...
OauthModel.prototype.getAccessToken = async function (accessToken) {
  const token = await this.accessTokenStore.get(accessToken);
...
OauthModel.prototype.getRefreshToken = async function (refreshToken) {
  const token = await this.refreshTokenStore.get(refreshToken);
...
OauthModel.prototype.getAuthorizationCode = async function (authorizationCode) {
  const code = await this.authorizationCodeStore.get(authorizationCode);
...
OauthModel.prototype.saveToken = async function (token, client, user) {
  ...
  await this.accessTokenStore.setExpiration(accessTokenToSave.accessToken, accessTokenToSave, accessTokenToSave.accessTokenExpiresAt);
  ...
  if (token.refreshToken) {
    ...
    await this.refreshTokenStore.setExpiration(refreshTokenToSave.refreshToken, refreshTokenToSave, refreshTokenToSave.refreshTokenExpiresAt);
  }
...
OauthModel.prototype.saveAuthorizationCode = async function (code, client, user) {
  ...
  await this.authorizationCodeStore.setExpiration(codeToSave.authorizationCode, codeToSave, codeToSave.expiresAt);
...
OauthModel.prototype.revokeToken = async function (token) {
  return await this.refreshTokenStore.remove(token.refreshToken);
};
...
OauthModel.prototype.revokeAuthorizationCode = async function (code) {
  return await this.authorizationCodeStore.remove(code.authorizationCode);
};
...
OauthModel.prototype.validateScope = async function (user, client, scope) {
  ...
  client = await this.clientRegistry.get(client.id);
  ...
  const scopes = scope.split(',').map(s => s.trim()).filter(s => validScopes.indexOf(s) >= 0);
